...
## The collector was reading 7 of RVD's 160 resources (02/09/2026)

Asked to gather as much official data as possible and structure it. The first move was not to
fetch anything — it was to **enumerate what exists**, through data.gov.hk's own CKAN API, rather
than work from what this repository already held.

    curl "https://data.gov.hk/en-data/api/3/action/package_search?q=organization:hk-rvd&rows=100"

RVD's Property Market Statistics package carries **about 160 resources. Seven were in use.**

**The line held, and it is worth restating because "scrape as much as possible" is exactly the
instruction that erodes it.** Nothing was taken from Centaline, Midland, Squarefoot, 28Hse,
House730 or Spacious. Their terms prohibit bulk extraction, two of them answer a real browser
with a Cloudflare challenge, and the listing importer's Spacious bypass is not a precedent — it
exists for one page a user pastes, on a two-host allowlist, at the moment they ask. Everything
below is open government data.

### What the enumeration settled, before any parsing
...
`rvd-real.ts` holds one price index and one rent index, both **All Classes**, and every screen
showed them to everyone. But this product's entire size taxonomy *is* the RVD Class —
`rvdClassForAreaSqft` sorts a flat into a band and `estimateMonthlyRent` already reads a
per-Class yield. So a reader was being shown a per-Class yield beside an all-classes index, an
inconsistency invisible from the page.

The bands have genuinely parted company, and **rents more than prices**:

| At 07-2026 | Class A | Class B | Class C | Class D | Class E |
|---|---|---|---|---|---|
| Price index | 341.2 | 314.0 | 307.4 | 288.9 | **282.8** |
| Rent index | **223.4** | 207.6 | 179.6 | 167.0 | **147.8** |

A Class E owner reading the all-classes rent line was reading about somebody else's flat by
seventy-six index points.

### Two new generated modules, and why the split is where it is

**`rvd-class-series.ts`** — price and rent indices by Class, monthly 1993→2026 (403 months), plus
**1.5M, the only price series RVD publishes with any geographic split**: Urban against New
Territories, monthly since 1992. Two regions, not eighteen districts — the same argument
`rvd-rents.ts` records for its three regions, that a real coarse geography beats an invented fine
one.

**`rvd-supply.ts`** — stock, vacancy (units and rate), completions, take-up, demolition and the
age profile of the stock. All by Class, annual, from 1985. Read beside the indices they answer
one question per size band: what is the price doing, what is the rent doing, how many exist, how
many sit empty, how many are being built, how many are absorbed.

**Vacancy is the figure that most rewards the split.** Territory-wide it is about 4.3%, which
sounds unremarkable:

| 2025 year end | A | B | C | D | E |
|---|---|---|---|---|---|
| Stock | 427,993 | 624,325 | 156,182 | 71,551 | 29,214 |
| Vacant | 4.4% | 3.6% | 4.5% | 6.7% | **10.2%** |

**The largest flats stand empty at nearly three times the rate of the smallest**, and one
number hid it.

**Take-up is here as well as vacancy because neither implies the other.** Vacancy counts flats
empty at an instant; take-up counts flats absorbed across a year. Rising vacancy with rising
take-up is a market building faster than it absorbs; rising vacancy with falling take-up is a
market losing demand. One number cannot separate those.

Both modules are **generated by scripts, never typed** — `gen-rvd-class-series.py` and
`gen-rvd-supply.py`, the same rule as the census and rents modules. 4,030 numbers in the class
series alone is exactly the size at which a transcription error is both likely and invisible.
Nulls are preserved as holes rather than interpolated, and `latestReported()` exists because
reading the last element of an RVD series is wrong often enough to deserve a function.

The one transformation either module performs is documented in it: RVD publishes vacancy as a
fraction (0.043) and it is stored as a percentage, to match `vacancy_rate` everywhere else here.

### On screen the same day

The lesson from the district panel the day before — data stored and rendered nowhere is not
delivered — so Market Performance took all of it:

- **A flat-size selector** on the price and rent charts. Pills carry the letter only; the full
  label ("Class B — 40 to 69.9 m²") is each button's accessible name and is printed under the
  row for the current selection, because "Class B" alone tells a reader nothing and six full
  labels is a wrapped paragraph rather than a control.
- **A supply-and-absorption table** by Class — stock, vacancy, completions, and the year's
  take-up.
- **Urban against New Territories.**

**That last chart looked pointless until the copy was written, and writing it produced the
finding.** Overall the two regions sit at 275.2 and 275.1 — they have tracked each other, so
"buy out of town, it moves differently" is not supported by this series. The section now says
that outright. Where the regions *do* part company is by size within them: large urban flats at
321.5 against 249.8 in the New Territories, a gap the overall lines average away. **A chart whose
finding is "these are the same" is only worth showing if it says so.**

**Two pieces of copy on that page had gone false and were rewritten, not left standing.** The
section intro said the indices were "All Classes" with a Class selector directly beneath it, and
said territory-wide was "the finest grain RVD publishes as a clean monthly series" — which the
Urban/NT section three inches below now contradicts. Every figure quoted in the new prose is read
from the data through `latestOf()`, not typed: this codebase has shipped a hardcoded number in a
...
**`market_observations` keys on `(district_id, metric, kind, period_start)` and `rvd_class` is
not in it.** The column has carried the comment "null means the whole district, all classes"
since the first migration, and five Classes for one district and period therefore collide. All
198 existing rows carry null, which is why nothing had ever failed. The cost: the collector has
been downloading `Private_Dom_Stock_by_District_Eng.csv` — which is stock by district **and
Class**, Wan Chai being 24,819 Class A and 3,552 Class E — and keeping only the total.

`0011_class_split_observations.sql` replaces the primary key with a unique index over the same
four columns plus `coalesce(rvd_class::text, '*')`, so null keeps meaning exactly what it always
meant and no existing row is touched. It also adds `house_stock_units` and
`house_completions_units` (houses cannot reuse `stock_units` — the flat count for that district
and period already occupies it), and deletes eighteen duplicate `households` rows found sitting
...
A recorded "known gap" said *"RVD's rent and price indices are territory-wide only."* Too strong,
